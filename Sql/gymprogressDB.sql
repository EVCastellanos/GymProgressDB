...
CREATE TABLE IF NOT EXISTS `ASIGNACIONES` (
  ...
  CONSTRAINT `FK_ASIGNACIONES_ENTRENADOR` FOREIGN KEY (`ID_ENTRENADOR`) REFERENCES `ENTRENADOR` (`ID_ENTRENADOR`),
...
CREATE TABLE IF NOT EXISTS `EPS` (
  ...
  `EAIL_EPS` varchar(100) CHARACTER SET utf16 COLLATE utf16_spanish_ci DEFAULT NULL COMMENT 'Correo electronico de contacto EPS',
...
CREATE TABLE IF NOT EXISTS `PROGRESO` (
  ...
  CONSTRAINT `FK_PROGRESO_RUTINA` FOREIGN KEY (`ID_RUTINA`) REFERENCES `RUTINA` (`ID_RUTINA`),
  CONSTRAINT `FK_PROGRESO_USUARIOS` FOREIGN KEY (`ID_USER`) REFERENCES `USUARIOS` (`ID_USER`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb3 COLLATE=utf8mb3_spanish_ci COMMENT='Progreso de los usuarios que asisten al gimnasio';
...
CREATE TABLE IF NOT EXISTS `TOMA_MEDIDAS` (
  ...
  CONSTRAINT `FK_TOMA_MEDIDAS_PROGRESO` FOREIGN KEY (`ID_PROGRESO`) REFERENCES `PROGRESO` (`ID_PROGRESO`),
